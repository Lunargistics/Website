...
interface ISpaceElementsNFT is IERC721, IERC2981 {
    ...
    function spaceElements(uint256 tokenId) external view returns (
        uint256, address, uint8, string memory, string memory,
...
contract SpaceElementsMarketplace is ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(address _spaceElementsNFT) 
        Ownable(msg.sender) 
    {
    ...
    function createOrder(
        ...
        for (uint i = 0; i < offer.length; i++) {
            require(
                spaceElementsNFT.ownerOf(offer[i].tokenId) == msg.sender,
                "Not token owner"
            );
        }
        
        orderHash = _hashOrder(
            msg.sender,
            offer,
            consideration,
            orderType,
    ...
    function settleAuction(uint256 tokenId) external nonReentrant {
        ...
        if (auction.currentBid >= auction.reservePrice && auction.currentBidder != address(0)) {
            ...
            (address royaltyReceiver, uint256 royaltyAmount) = spaceElementsNFT.royaltyInfo(tokenId, auction.currentBid);
