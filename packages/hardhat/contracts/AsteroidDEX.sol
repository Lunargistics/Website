...
contract AsteroidDEX is Ownable {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pool) {
        ...
        allPools.push(Pool({ poolAddress: pool, token0: token0, token1: token1, createdAt: block.timestamp }));
    ...
    function addLiquidity(
        ...
        (amountA, amountB) = _calculateOptimalAmounts(pool, amountADesired, amountBDesired, amountAMin, amountBMin);
