...
function testOrbitalCalculationPerformance() {
  ...
  console.log(`✅ Orbital calculations: ${operationsPerSecond} ops/sec (${duration}ms for ${calculations} calculations)`);
...
async function testConcurrentPerformance() {
  ...
  const operations = Array(concurrentOperations).fill().map((_, i) => 
    simulateAsyncOperation(Math.random() * 20 + 5) // 5-25ms operations
  );
  ...
  console.log(`✅ Concurrent operations: ${operationsPerSecond} ops/sec (${duration}ms for ${concurrentOperations} operations)`);
