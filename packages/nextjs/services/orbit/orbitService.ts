...
// import { parseTLE } from "tle.js"; // Not currently used
...
// const EARTH_FLATTENING = 1 / 298.257223563; // Not currently used
...
class OrbitService {
   ...
   */
  propagateFromTLE(tleLine1: string, tleLine2: string, date: Date): StateVector {
  ...
  stateToElements(state: StateVector): OrbitalElements {
    ...
    // Eccentricity vector
    const ex =
      (state.velocity.y * (state.position.z * state.velocity.y - state.position.y * state.velocity.z) -
        state.velocity.z * (state.position.x * state.velocity.z - state.position.z * state.velocity.x)) /
        EARTH_MU -
      state.position.x / r;
    const ey =
      (state.velocity.z * (state.position.x * state.velocity.z - state.position.z * state.velocity.x) -
        state.velocity.x * (state.position.z * state.velocity.y - state.position.y * state.velocity.z)) /
        EARTH_MU -
      state.position.y / r;
    const ez =
      (state.velocity.x * (state.position.y * state.velocity.x - state.position.x * state.velocity.y) -
        state.velocity.y * (state.position.x * state.velocity.z - state.position.z * state.velocity.x)) /
        EARTH_MU -
      state.position.z / r;
  ...
  elementsToState(elements: OrbitalElements): StateVector {
    ...
    const position = {
      x:
        (cosRaan * cosW - sinRaan * sinW * cosI) * posPerifocal.x +
        (-cosRaan * sinW - sinRaan * cosW * cosI) * posPerifocal.y,
      y:
        (sinRaan * cosW + cosRaan * sinW * cosI) * posPerifocal.x +
    ...
    const velocity = {
      x:
        (cosRaan * cosW - sinRaan * sinW * cosI) * velPerifocal.x +
        (-cosRaan * sinW - sinRaan * cosW * cosI) * velPerifocal.y,
      y:
        (sinRaan * cosW + cosRaan * sinW * cosI) * velPerifocal.x +
  ...
  calculateEclipses(
    ...
    // const sunRadius = 695700; // km - Not currently used
   ...
   */
  planManeuver(currentOrbit: OrbitalElements, targetOrbit: OrbitalElements, maneuverTime: Date): ManeuverPlan {
  ...
  calculateSensorFootprint(
    ...
    _pointingVector?: { x: number; y: number; z: number },
  ): SensorFootprint {
    const r = Math.sqrt(
      satellitePosition.position.x ** 2 + satellitePosition.position.y ** 2 + satellitePosition.position.z ** 2,
    );
   ...
   */
  propagateFromElements(elements: OrbitalElements, targetTime: Date): StateVector {
    ...
    const mDot =
      ...
      0.75 *
        n *
        J2 *
        Math.pow(EARTH_RADIUS / p, 2) *
        Math.sqrt(1 - elements.eccentricity ** 2) *
        (3 * Math.cos(incRad) ** 2 - 1);
    ...
    const nu = (Math.atan2(Math.sqrt(1 - e ** 2) * Math.sin(E), Math.cos(E) - e) * 180) / Math.PI;
