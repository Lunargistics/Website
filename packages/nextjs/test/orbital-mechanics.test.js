...
async function testOrbitalCalculations() {
  ...
  try {
    ...
    if (propagation.length > 0) {
      ...
      console.log(`  Position: (${result.position.x.toFixed(1)}, ${result.position.y.toFixed(1)}, ${result.position.z.toFixed(1)}) km`);
      console.log(`  Velocity: (${result.velocity.vx.toFixed(3)}, ${result.velocity.vy.toFixed(3)}, ${result.velocity.vz.toFixed(3)}) km/s`);
    ...
    const passes = await orekitService.calculateGroundPasses(
      ISS_ORBIT_ELEMENTS, 
      groundStation, 
      startTime, 
      endTime
