...
function validateTLELine1(line: string): boolean {
  // Check basic format
  if (
    !/^1 \d{5}[A-Z ] \d{2}\d{3}[A-Z ]{3} \d{5}\.\d{8} [+-]?\.?\d{8} [+-]?\d{5}[+-]\d [+-]?\d{5}[+-]\d \d \d{4}/.test(
      line,
    )
  ) {
    return false;
...
const LaunchWindowRequestSchema = z
  .object({
...
const SolarPowerRequestSchema = z
  .object({
    orbit: OrbitDataSchema,
    spacecraftConfig: z.object({
      solarPanelArea: z
        .number()
        .min(0.1, "Solar panel area must be at least 0.1 m²")
        .max(10000, "Solar panel area too large"),
      solarPanelEfficiency: z
        .number()
        .min(0.05, "Efficiency too low")
        .max(0.45, "Efficiency exceeds current technology"),
      batteryCapacity: z
        .number()
        .min(1, "Battery capacity must be at least 1 Wh")
        .max(1000000, "Battery capacity too large"),
      ...
      dischargeEfficiency: z.number().min(0.8).max(0.98).default(0.9),
...
export function validateOrbitalParameters(orbit: any): { valid: boolean; warnings: string[]; errors: string[] } {
  ...
  if (orbit.semiMajorAxis < 6578137) {
    // 200km altitude minimum
    warnings.push("Semi-major axis indicates very low orbit (below 200km)");
  }

  if (orbit.semiMajorAxis > 384400000) {
    // Beyond lunar orbit
