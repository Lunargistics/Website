...
import { veniceAI, VeniceAnalysis } from "~~/services/veniceAI";
import { 
  SparklesIcon, 
  ShieldCheckIcon, 
...
export const AIAnalysisPanel = ({ documents = [], missionName, onAnalysisComplete }: AIAnalysisPanelProps) => {
  ...
  const getRiskIcon = (level: string) => {
    switch (level) {
      case 'low': return <ShieldCheckIcon className="h-5 w-5" />;
      case 'medium': return <ExclamationTriangleIcon className="h-5 w-5" />;
      case 'high': return <ExclamationTriangleIcon className="h-5 w-5 text-error" />;
