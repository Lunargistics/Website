      ...
      try {
        ...
        switch (action) {
          case "propagateOrbit": {
            ...
            const result = await OrekitService.propagateOrbit(
              validation.data,
              targetTime,
              options
            );
          ...
          case "calculateManeuver": {
            ...
            const result = await OrekitService.calculateManeuver(
              ...
              validation.data.maneuverType
            );

            // Calculate fuel requirements if spacecraft mass provided
            const spacecraftMass = body.spacecraftMass ? InputSanitizer.sanitizeNumber(body.spacecraftMass, 1, 100000) : null;
            const specificImpulse = body.specificImpulse ? InputSanitizer.sanitizeNumber(body.specificImpulse, 100, 500) : 300;
          ...
          case "calculateLaunchWindow": {
            ...
            const searchEnd = body.searchEnd ? new Date(body.searchEnd) : new Date(searchStart.getTime() + 30 * 86400000); // 30 days
          ...
          case "calculateStationKeeping": {
            ...
            const tolerances = InputSanitizer.sanitizeObject(body.tolerances);
            const duration = InputSanitizer.sanitizeNumber(body.duration, 1, 365); // Max 1 year

            const result = await OrekitService.calculateStationKeeping(
              orbitValidation.data,
              tolerances,
              duration
            );
