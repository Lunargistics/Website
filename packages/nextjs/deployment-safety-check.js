...
function checkDeploymentFiles() {
  ...
  const requiredFiles = ["package.json", "next.config.ts", "vercel.json", ".env.example", ".env.production.example"];
...
function checkEnvironmentDocumentation() {
  ...
  try {
    ...
    const criticalVars = ["NEXTAUTH_SECRET", "MONGODB_URI", "NEXT_PUBLIC_PINATA_JWT", "NEXTAUTH_URL"];
...
function checkBlueGreenCapabilities() {
  ...
  const hasHealthCheck = fs.existsSync("app/api/health") || fs.readFileSync("package.json", "utf8").includes("health");
  if (hasHealthCheck) {
